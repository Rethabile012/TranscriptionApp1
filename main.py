...
def predict(audio_file, model, encoder, decoder):
    ...
    # FIX: pass as NumPy array (T, num_features)
...
class SpeechToTextApp:
    ...
    def show_upload_screen(self):
        ...
        tk.Entry(self.root, textvariable=self.filepath_var, width=50).pack()
        tk.Button(self.root, text="Browse", command=self.browse_file).pack(pady=5)
        tk.Entry(self.root, textvariable=self.filename_var, width=50).pack()
        ...
        # Create matplotlib figure for waveform and spectrogram
        ...
        tk.Label(self.root, text="Transcription", font=("Arial", 12)).pack(pady=5)
        self.transcription_text = tk.Text(self.root, height=6, width=80)
        self.transcription_text.pack()
        
        # Download button
        self.download_btn = tk.Button(self.root, text="Download Transcription", 
                                      command=self.download_transcription, state=tk.DISABLED)
    ...
    def plot_audio_visualizations(self, file_path):
        ...
        try:
            ...
            img = librosa.display.specshow(D, sr=sr, x_axis='time', y_axis='hz', ax=self.ax2)
            self.ax2.set_title('Spectrogram')
            self.fig.colorbar(img, ax=self.ax2, format='%+2.0f dB')
    ...
    def upload_and_transcribe(self):
        ...
        try:
            # Show visualizations
            self.plot_audio_visualizations(file_path)
